...
function Toggle({ on, onChange, label }: { on: boolean; onChange: (v: boolean) => void; label: string }) {
  return (
    ...
    >
      <span className={`block w-5 h-5 rounded-full bg-white shadow transition-transform ${on ? "translate-x-4" : "translate-x-0"}`} />
    </button>
  );
}
...
function SettingsPage() {
  ...
  return (
    <div className="space-y-6 max-w-4xl">
      ...
      <Card>
        <CardHeader title="Wallet Connection" subtitle="Connected Solana wallets" action={<Wallet className="w-4 h-4 text-muted-foreground" />} />
          ...
          {connected && address ? (
            <div className="flex items-center gap-3 p-3 rounded-lg border border-border bg-elevated/40">
              ...
              <div className="flex-1 min-w-0">
                ...
                <div className="text-xs font-mono text-muted-foreground truncate">{shortAddress(address)}</div>
              </div>
              <Badge variant="primary">Primary</Badge>
              <CredButton size="sm" variant="outline" onClick={disconnect}>Disconnect</CredButton>
            </div>
          ) : (
            <div className="p-4 rounded-lg border border-dashed border-border text-sm text-muted-foreground text-center">
              No wallet connected. Use the Connect Wallet button in the top-right to link Phantom, Solflare, or Backpack.
            </div>
          )}
        </div>
      </Card>
      ...
      <Card>
        <CardHeader title="Notifications" subtitle="What you want to hear about" action={<Bell className="w-4 h-4 text-muted-foreground" />} />
        <ToggleList items={[
      ...
      <Card>
        <CardHeader title="Theme" subtitle="Visual preferences" action={<Palette className="w-4 h-4 text-muted-foreground" />} />
        <div className="px-5 pb-5 grid grid-cols-3 gap-3">
          {["Midnight", "Cobalt", "Carbon"].map((t, i) => (
            <button
              key={t}
              onClick={() => { setTheme(i); toast.success(`${t} theme selected`); }}
            ...
            >
              <div className="h-12 rounded-md mb-2 btn-primary" style={{ opacity: 0.8 - i * 0.2 }} />
              <div className="text-sm font-medium">{t}</div>
              <div className="text-xs text-muted-foreground">{theme === i ? "Active" : "Available"}</div>
            </button>
          ))}
        </div>
      </Card>
      ...
      <Card>
        <CardHeader title="Security" subtitle="Account & API protections" action={<ShieldCheck className="w-4 h-4 text-success" />} />
        <ToggleList items={[
