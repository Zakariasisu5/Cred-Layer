...
import { Activity, AlertTriangle, ArrowUpRight, Cpu, ShieldCheck, Sparkles, Zap } from "lucide-react";
...
export const Route = createFileRoute("/dashboard")({
  head: () => ({
    meta: [
      ...
      { name: "description", content: "Wallet trust score, AI confidence, blockchain activity, and risk alerts." },
    ],
  }),
  component: Dashboard,
});
    ...
    <div className="space-y-6">
      <div className="flex flex-wrap items-end justify-between gap-3">
        <div>
          ...
          <p className="text-sm text-muted-foreground mt-1">
            <span className="font-mono">{connected && address ? shortAddress(address) : "Not connected"}</span> · Solana Mainnet
          </p>
        </div>
        <div className="flex items-center gap-2">
          <Badge variant="success"><ShieldCheck className="w-3 h-3" /> Verified</Badge>
          <Badge variant="primary"><Sparkles className="w-3 h-3" /> AI Confidence 94%</Badge>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-5">
        ...
        <Card className="lg:col-span-2">
          <CardHeader title="Reputation Trend" subtitle="Last 24 hours · Trust score vs AI confidence" action={<Badge variant="primary"><Activity className="w-3 h-3" /> Live</Badge>} />
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-5">
        <Card><Stat label="Wallet Age" value="2.4 yrs" delta="Tier 3" /></Card>
        <Card><Stat label="Total Value" value="$48.2K" delta="3.2%" deltaType="up" /></Card>
        <Card><Stat label="Tx Count (30d)" value="412" delta="12%" deltaType="up" /></Card>
        <Card><Stat label="Risk Flags" value="0" delta="Clean" deltaType="up" /></Card>
      </div>

      <div className="grid grid-cols-1 lg:grid-cols-3 gap-5">
        <Card className="lg:col-span-2">
          <CardHeader title="Recent Blockchain Activity" subtitle="Latest transactions and contract interactions" />
                ...
                {activity.map((a, i) => (
                  <tr key={i} className="border-b border-border/50 hover:bg-accent/30 transition">
                    ...
                    <td className="px-5 py-3">
                      <Badge variant={a.risk === "low" ? "success" : a.risk === "med" ? "warning" : "danger"}>
                        {a.risk.toUpperCase()}
                      </Badge>
                    </td>
        ...
        <Card>
          <CardHeader title="Suspicious Activity" subtitle="AI-detected anomalies" action={<Badge variant="danger"><AlertTriangle className="w-3 h-3" /> 2 New</Badge>} />
          <div className="px-5 pb-5 space-y-3">
            {[
              { t: "Unverified program interaction", d: "Program ID: 4nT…vqz", v: "warning" as const, icon: AlertTriangle },
              { t: "Unusual outbound transfer pattern", d: "3 wallets, 5 minutes apart", v: "danger" as const, icon: Zap },
              { t: "AI flagged Sybil cluster proximity", d: "Confidence 71%", v: "warning" as const, icon: Cpu },
            ].map((x, i) => (
              <div key={i} className="p-3 rounded-lg border border-border bg-elevated/50 flex gap-3">
                <div className={`w-8 h-8 rounded-md grid place-items-center bg-${x.v === "danger" ? "danger" : "warning"}/15`}>
                  <x.icon className={`w-4 h-4 ${x.v === "danger" ? "text-danger" : "text-warning"}`} />
                </div>
        ...
        <div className="h-64 px-2 pb-3">
          <ResponsiveContainer width="100%" height="100%">
            <BarChart data={interactions}>
              ...
              <Bar dataKey="nft" fill="var(--color-primary)" fillOpacity={0.45} radius={[6, 6, 0, 0]} />
            </BarChart>
          </ResponsiveContainer>
        </div>
      </Card>
