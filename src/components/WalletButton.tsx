          ...
          <>
            ...
            <div className="absolute right-0 mt-2 w-64 rounded-xl glow-card p-2 z-50">
              <div className="px-3 py-2">
                <div className="text-[10px] uppercase tracking-widest text-muted-foreground">{walletName}</div>
    ...
    <>
      ...
      {open && (
        <div className="fixed inset-0 z-50 grid place-items-center bg-background/70 backdrop-blur-sm p-4" onClick={() => setOpen(false)}>
          <div className="w-full max-w-sm rounded-2xl glow-card p-5" onClick={(e) => e.stopPropagation()}>
            <div className="text-base font-semibold">Connect a wallet</div>
            <div className="text-xs text-muted-foreground mt-1">Solana Mainnet · choose your provider</div>
            <div className="mt-4 space-y-2">
              {wallets.map((w) => (
                <button
                  key={w.name}
                  onClick={async () => { await connect(w.name); setOpen(false); }}
                  disabled={connecting}
                ...
                >
                  <div className={`w-9 h-9 rounded-md grid place-items-center bg-gradient-to-br ${w.accent}`}>
                    <Wallet className="w-4 h-4 text-white" />
                  </div>
                  <div className="text-sm font-medium flex-1 text-left">{w.name}</div>
                  {connecting ? <Loader2 className="w-4 h-4 animate-spin text-muted-foreground" /> : <Check className="w-4 h-4 text-muted-foreground/40" />}
                </button>
              ))}
            </div>
            <button onClick={() => setOpen(false)} className="mt-4 w-full h-10 rounded-lg text-sm border border-border hover:bg-accent">
              Cancel
            </button>
          </div>
        </div>
